...
QueueSemaphore::wait()
{
  ...
  {               // else return immediately.
    if(strncmp(name,"mteasse",7) == 0){
    // cout << "If there isn't any signal, then wait in queue. else return immediately." << endl;
  }
    Thread::current()->waitFor(this);
    if(strncmp(name,"mteasse",7) == 0){
    // cout << "QueueSemaphore::wait waitFor finished" << endl;
  }
    waitQueue.Append(Thread::current());
    if(strncmp(name,"mteasse",7) == 0){
    // cout << "QueueSemaphore::wait Append finished" << endl;
  }
    Scheduler::instance().runFirstReady();
     if(strncmp(name,"mteasse",7) == 0){
    // cout << "QueueSemaphore::wait runFirstReady finished" << endl;
  }
  }
  else
  {
