...
LLCInPacket::decode() {
    ...
    if (myDestinationAddress == Ethernet::instance().myAddress()) {
        //Ethernet::instance().bytesOffsetPrintOut(myData,6,6); // SRC SHOULD BE 08-60-6E-7F-40-66
        //cout << "LLCInPacket correct ethernet address" << endl;
        //byte temp = (*(myData + 20);
        //cout << "length " << myLength << " typelen 0x" << hex << myTypeLen << dec << " (" << myTypeLen << ")" << endl;

        //Ethernet::instance().bytesOffsetPrintOut((myData + 20),0,1);
        if ((myTypeLen == 0x800) && (myLength > 28) && (*(myData + 20) == 8)) {
            ...
            //cout << "icmp echo, icmp_seq=" << icmpSeq << endl;
            // create a resonse...
